...
const emailReducer = (state, action) => {
  ...
  // dispatch 함수가 전달한 액션 객체의 타입에 따라 변경할 상태값 반환
...
const Login = ({ onLogin }) => {
  ...
  */
  ...
  // 기존에 email 상태변수 제거함
  // 상태값이 필요하다면 -> reducer에서 제동되는 상태값을 활용
  //emailState에서 isValid 프로퍼티를 디스트럭처링함 (프로퍼티 바로사용 x)
  const { isValid: emailIsValid } = emailState;
  const { isValid: pwIsValid } = pwState;

  // 입력란(이메일, 비밀번호)을 모두 체크하여 form의 버튼 disabled를 해제하는
  // 상태변수 formValid의 사이드 이펙트를 처리하는 영역
  useEffect(() => {
    // formIsValid의 유효성 검증을 일부러 1초 뒤에 실행하도록 setTimeout을 사용
    // 1초 이내에 새로운 입력갑시 들어옴 -> 상태 변경 -> 재 렌더링이 진행 되면서
    // useEffect가 또 호출이 됨
    ...
    // 의존성 배열에 상태변수를 넣어주면 그 상태 변수가 바뀔 때마다
    // useEffect가 재 실행됨
  }, [emailIsValid, pwIsValid]);

  const emailChangeHandler = (e) => {
    // reducer의 상태 변경은 dispatch 함수를 통해서 처리
    // dispatch 함수의 매개값 객체의 key 는 정해진 것이 아닌, reducer 함수에서
    // 구분하기 위해 붙혀주는 이름
    // 프로퍼티의 key 와 value는 자유롭게 줄 수 있음
    dispatchEmail({
      type: 'USER_INPUT',
      val: e.target.value,
    });
